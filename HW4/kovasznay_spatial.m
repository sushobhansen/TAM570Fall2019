...
for N=Nstart:Nmax
    ...
    error(N-Nstart+1,1) = max(max(abs(sqrt((U12-U1ex).^2+(U22-U2ex).^2))));
end

%% Plot error
semilogy([Nstart:Nmax]',error,'b'); hold on;
semilogy([Nstart:Nmax]',[Nstart:Nmax]'.^(-2),'r--');
semilogy([Nstart:Nmax]',[Nstart:Nmax]'.^(-4),'k.-');
xlabel('N'); ylabel('Max Abs Error');
title('Spatial convergence for Kovasznay');
legend('Solution','O(n^2)','O(n^4)');
% figure(1)
% contour(X,Y,vort(U11,U21,Dhx,Dhy),30);
% hold on
% quiver(X,Y,U11,U21); hold on; xlabel('x'); ylabel('y');
% hold off
% title('Vorticity and velocity vectors');
% 
% figure(2)
...
% contourf(X,Y,error); hold on; xlabel('x'); ylabel('y');colorbar; title('Error');
