...
for N=Nstart:Nmax
    ...
    for i=1:nsteps
        ...
        if mod(i,100)==0
           ...
           disp(sprintf('For N = %d of %d, ran iteration %d of %d',N,Nmax,i,nsteps));
        end
        if i==1 %BDF1/EXT1
    ...
    error(N-Nstart+1,1) = max(max(abs(sqrt((U12-U1exf).^2+(U22-U2exf).^2))));
end

%% Plot error
semilogy([Nstart:Nmax]',error,'b'); hold on;
semilogy([Nstart:Nmax]',[Nstart:Nmax]'.^(-2),'r--');
semilogy([Nstart:Nmax]',[Nstart:Nmax]'.^(-3),'k.-');
xlabel('N'); ylabel('Max Abs Error');
title('Spatial convergence for Walsh');
legend('Solution','O(n^2)','O(n^3)');
axis square;
