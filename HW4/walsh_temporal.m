...
%% Boundaries
lambda = -25;
U1ex0=cos(3*X).*cos(4*Y)+sin(5*Y);
U2ex0=0.75*sin(3*X).*sin(4*Y)+cos(5*X);

%Set BCs to exact solution
U1b0 = 0*X; 
%Left  & Right
U1b0(1,:) =U1ex0(1,:);  U1b0(end,:) =U1ex0(end,:);
%Down  &  Up
U1b0(:,1) = U1ex0(:,1); U1b0(:,end) = U1ex0(:,end);
...
U2b0 = 0*X; 
%Left & Right
U2b0(1,:)=U2ex0(1,:); U2b0(end,:)=U2ex0(end,:);
%Down & Up
U2b0(:,1)=U2ex0(:,1); U2b0(:,end)=U2ex0(:,end);
...
for j=1:size(CFL,1)
    ...
    for i=1:nsteps
        %Time-varying BC
        U1b = U1b0*exp(lambda*nu*(i)*dt);
        U2b = U2b0*exp(lambda*nu*(i)*dt);
        %Display
        if mod(i,100)==0
            %contourf(X,Y,sqrt(U10.^2+U20.^2)); colorbar; pause(0.1);
           disp(sprintf('For CFL = %f (%d of %d), ran iteration %d of %d',CFL(j),j,size(CFL,1),i,nsteps)); 
        end
        if i==1 %BDF1/EXT1
           ...
           %Boostrap BDF1 with exact solution
           U11 = U1ex0*exp(lambda*nu*(i)*dt);
           U21 = U2ex0*exp(lambda*nu*(i)*dt);
        ...
        elseif i==2 %BDF2/EXT2
           ...
           %Boostrap BDF2 with exact solution
           U12 = U1ex0*exp(lambda*nu*(i)*dt);
           U22 = U2ex0*exp(lambda*nu*(i)*dt);
    ...
    U1exf = U1ex0*exp(lambda*nu*tmax);
    U2exf = U2ex0*exp(lambda*nu*tmax);
    dts(j) = dt;
    error(j) = max(max(abs(sqrt((U12-U1exf).^2+(U22-U2exf).^2))));
end

% semilogy(dts,error,'b'); hold on;
% semilogy(dts,dts.^(3),'r--');
loglog(dts,error,'k.-'); hold on;
loglog(dts,1e-3*dts.^(1),'b-');
loglog(dts,1e3*dts.^(3),'r--');
xlabel('\Delta t'); ylabel('Max Abs Error');
title('Temporal convergence for Walsh');
legend('Solution','O(\Delta t)','O(\Delta t^3)');
...
% figure(3)
% subplot(2,1,1);
% contourf(X,Y,sqrt(U1exf.^2+U2exf.^2)); hold on; xlabel('x'); ylabel('y');colorbar; 
...
% 
% figure(4)
% error = abs(sqrt((U12-U1exf).^2+(U22-U2exf).^2));
