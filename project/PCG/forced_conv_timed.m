...
%% Matrices for FEM preconditioner
Jvx = interp_mat(xp,x);
Jvy = interp_mat(yp,y);
Jvxi =interp_mat(xp,Rx1*x);
Jvyi = interp_mat(yp,Ry1*y);
...
%% LU Factorization for FEM Preconditioner
[x1,w1] = zwgll(1);
B1 = spdiags(w1,0,2,2);
D1 = deriv_mat(x1);
AA1 = D1'*B1*D1; AA1 = 0.5*(AA1+AA1');
BB1 = B1;
xu = Rx1*x; yu = Ry1*y;
AL = kron(diag((2./diff(yu))),diag(diff(xu)./2));
AL = kron(AL,kron(AA1,BB1));
AL = AL + kron(kron(diag(diff(yu)./2),diag(2./diff(xu))),kron(BB1,AA1));
Nu = size(xu,1);
Q = zeros(4*(Nu-1)*(Nu-1),Nu^2);
j = 1; k = Nu + 1;
for i =1:4*(Nu-1)*(Nu-1)
...
Q = sparse(Q);
precondveci = Q'*AL*Q;
precondveci = precondveci(1:end-1,1:end-1);
[LL,UU] = lu(precondveci);
...
%        pause(0.1)
    ...
    if i==1 %BDF1/EXT1
        ...
        [rho1star,rhoiter] = solve(preconflag_U,brtilde,beta(1),Bhx,Bhy,nu,dt,Ahx,Ahy,Rxr,Ryr);
        ...
        [dU1star,U1iter] = solve(preconflag_U,b1tilde,beta(1),Bhx,Bhy,nu,dt,Ahx,Ahy,Rx1,Ry1);
        ...
        [dU2star,U2iter] = solve(preconflag_U,b2tilde,beta(1),Bhx,Bhy,nu,dt,Ahx,Ahy,Rx2,Ry2);
       
        %[U1,U2,dp] = incompress(dU1star,dU2star,dt,beta(1),U1b,U2b,SSx,SSy,DDinv,Jpx,Jpy,Bhx,Bhy,Dhx,Dhy,Rx1,Ry1,Rx2,Ry2);
        [U1,U2,dp,piter] = incompress_solve(preconflag_p,Aflag,beta(1),dt,x,y,AAx,AAy,BBx,BBy,dU1star,dU2star,U1b,U2b,Rx1,Ry1,Rx2,Ry2,Dhx,Dhy,Bhx,Bhy,Jpx,Jpy,Jvx,Jvy,Jvxi,Jvyi,LL,UU);
    ...
    elseif i==2 %BDF2/EXT2
        ...
        [rho2star,rhoiter] = solve(preconflag_U,brtilde,beta(1),Bhx,Bhy,nu,dt,Ahx,Ahy,Rxr,Ryr);
