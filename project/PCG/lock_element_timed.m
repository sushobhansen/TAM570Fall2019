...
BB1 = B1;
xu = Rx1*x; yu = Ry1*y;
AL = kron(sparse(diag(2./diff(yu))),sparse(diag(diff(xu)./2)));
AL = kron(AL,kron(AA1,BB1));
AL = AL + kron(kron(sparse(diag(diff(yu)./2)),sparse(diag(2./diff(xu)))),kron(sparse(BB1),AA1));
Nu = size(xu,1);
...
%        pause(0.1)
    ...
    if i==1 %BDF1/EXT1
        ...
        rho1star = solve(preconflag_U,brtilde,beta(1),Bhx,Bhy,nu,dt,Ahx,Ahy,Rxr,Ryr);
        ...
        dU1star = solve(preconflag_U,b1tilde,beta(1),Bhx,Bhy,nu,dt,Ahx,Ahy,Rx1,Ry1);
        ...
        dU2star = solve(preconflag_U,b2tilde,beta(1),Bhx,Bhy,nu,dt,Ahx,Ahy,Rx2,Ry2);
       
        %[U1,U2,dp] = incompress(dU1star,dU2star,dt,beta(1),U1b,U2b,SSx,SSy,DDinv,Jpx,Jpy,Bhx,Bhy,Dhx,Dhy,Rx1,Ry1,Rx2,Ry2);
        [U1,U2,dp] = incompress_solve(preconflag_p,Aflag,beta(1),dt,x,y,AAx,AAy,BBx,BBy,dU1star,dU2star,U1b,U2b,Rx1,Ry1,Rx2,Ry2,Dhx,Dhy,Bhx,Bhy,Jpx,Jpy,Jvx,Jvy,Jvxi,Jvyi,LL,UU);
    ...
    elseif i==2 %BDF2/EXT2
        ...
        rho2star = solve(preconflag_U,brtilde,beta(1),Bhx,Bhy,nu,dt,Ahx,Ahy,Rxr,Ryr);
